...
def predict(models: Dict[str,Union[InterceptModel, CyclicGeneralizedLinearModel, CyclicalGradientBooster]], test_data: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    n_dim = len([
        col for col in test_data.columns if col.startswith("theta")
    ])
