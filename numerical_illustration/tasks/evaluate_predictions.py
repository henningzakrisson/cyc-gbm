...
def evaluate_predictions(train_data: pd.DataFrame, test_data: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, plt.Figure]:
    """
    ...
    for data_set,data_name in zip([train_data, test_data], metrics.columns):
        for model_name in model_names:
            if model_name == "true":
                ...
                theta_cols = [col for col in data_set.columns if col.startswith(model_name + "_theta_")]
    ...
    data_sorted_mean['y'].rolling(window=100, min_periods=1).mean().plot(ax=ax[0], label="True")
    ...
    if 'true' in model_names:
        ...
        (data_sorted_var['y'] - data_sorted_var['theta_0']).pow(2).rolling(window=100, min_periods=1).mean().plot(ax=ax[1], label="True")
    else:
        data_sorted_var = train_data.sort_values(by="y").reset_index(drop=True)
        (data_sorted_var['y'] - data_sorted_var['y'].rolling(window=100, min_periods=1).mean()).pow(2).rolling(window=100, min_periods=1).mean().plot(ax=ax[1], label="True")
